...
export class CreateRolePermissionTable1740050588482
  implements MigrationInterface
{
  name = 'CreateRolePermissionTable1740050588482';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`role_permission\` (\`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, \`role_permission_id\` int NOT NULL AUTO_INCREMENT, \`role_id\` int NULL, \`module_id\` int NULL, \`create\` tinyint NULL, \`update\` tinyint NULL, \`read\` tinyint NULL, \`delete\` tinyint NULL, PRIMARY KEY (\`role_permission_id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
